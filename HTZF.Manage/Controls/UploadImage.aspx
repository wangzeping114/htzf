<%@ Page Language="C#" AutoEventWireup="true" CodeFile="UploadImage.aspx.cs" Inherits="Controls_UploadImage" %>
...
<head runat="server">
    ...
    <link type="text/css" rel="Stylesheet" href="<%= ResolveUrl("~/Controls/styles/style.css?Version=1") %>" />
    <link type="text/css" rel="Stylesheet" href="<%= ResolveUrl("~/Controls/styles/pintuer.css") %>" />
    ...
    <style type="text/css">
        ...
        p {margin-bottom:5px; font-size:12px;line-height:18px;word-break:break-word;}
...
<script type="text/javascript">
    ...
    (function (window) {
        ...
        var file = $("#fileUp"),
            ...
            //11为特殊数字  圆形截图
            if (yscale == 11&&xscale==11) {
            $("#preview-pane").addClass("addPicClass");
            }
        ...
        function initJcrop(sw, sh) {
            ...
            uploadImage.Jcrop({
                ...
                var totalGraph='<%=totalGraph %>';
                if(totalGraph=="1")
                {                
                  $(".jcrop-holder").find("div")[0].style.display = "none";
                  $(".preview-imgsize:eq(1)", $preview).html('裁切尺寸--宽：<b class="text-red">' + original.w + '</b>；高：<b class="text-red">' + original.h + '</b>');
                }
            });
        }
        ...
        function doTemp() {
            $("#formFile").ajaxSubmit({
                url: ConApiUrl + "api/Controls/CreateImage/CreateTemp",
                type: "post",
                dataType: "JSON",
                ...
                success: function (data) {
                    ...
                    if (res_arr[0] == "true") {
                        ...
                        $("#msg").addClass("red").html(res_arr[1]).show();
        ...
        function doUpload() {
            $("#formFile").ajaxSubmit({
                url: ConApiUrl + "api/Controls/CreateImage/UploadImgs",
                type: "post",
                dataType: "JSON",
                beforeSend: function (request) {
                    request.setRequestHeader("appKeyId", puMInfo.appKeyId);
